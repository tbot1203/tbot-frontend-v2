...
import { House, ChatText, Prohibit, Monitor, Key, List, TwitterLogo, SignOut, ChartLine  } from "phosphor-react";
import './style.css'
...
export default function Home() {
    ...
    useEffect(() => {
        const fetchTweets = async () => {
            ...
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/tweets/get-posted-tweets/${twitterId}`);
                    ...
                    <Container fluid className="py-4">
                        <Row>
                            <div className="col-12 col-md-5">
                                <h5 className="dashboard-title">Dashboard <span className="mensajes-title">&gt; Posted Tweets</span></h5>
                            </div>
                        ...
                        {tweets.length === 0 ? (
                            <Alert variant="info" className="alertme text-center">No available tweets</Alert>
                        ) : (
                            [...new Map(tweets.map(tweet => [tweet.tweet_text, tweet])).values()].map((tweet) => (
                            <Col key={tweet.id} md={6} className="col-12 mb-3">
